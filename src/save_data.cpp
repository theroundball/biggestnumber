...
namespace
{
    ...
    constexpr int SAVE_DATA_VERSION_V16 = 16;
    ...
    constexpr int SAVE_DATA_CARD_COUNT_V13 = 70;
    constexpr int SAVE_DATA_CARD_COUNT_V18 = 71;
    constexpr int SAVE_DATA_TRINKET_COUNT_V10 = 6;
    static_assert(int(CardType::COUNT) == 72);
    ...
    int save_data_map_v11_card_index(int old_index)
    {
        switch(old_index)
        {
        case 44:  // SEEDS
        case 57:  // DOUBLE_TIME
        case 64:  // STAT_CYCLES_ADD
        case 65:  // STAT_CYCLES_MUL
        case 66:  // STAT_FLASH_ADD
        case 67:  // STAT_FLASH_MUL
        case 68:  // STAT_DRAWN_ADD
        case 69:  // STAT_DRAWN_MUL
        case 70:  // STAT_EXILE_ADD
        case 71:  // STAT_EXILE_MUL
        case 72:  // STAT_DISCARD_ADD
        case 73:  // STAT_DISCARD_MUL
            return -1;
        ...
        for(int stat_index = 64; stat_index <= 73; ++stat_index)
        {
            if(old_index > stat_index)
            {
                ++shift;
            }
        }
    ...
    void save_data_migrate_counts_v11_to_v12(uint8_t* out_counts, const uint8_t* old_counts)
    {
        for(int type_index = 0; type_index < int(CardType::COUNT); ++type_index)
        {
            out_counts[type_index] = 0;
        }

        for(int old_index = 0; old_index < SAVE_DATA_CARD_COUNT_V11; ++old_index)
        {
            const int new_index = save_data_map_v11_card_index(old_index);

            if(new_index >= 0 && new_index < int(CardType::COUNT))
            {
                out_counts[new_index] = old_counts[old_index];
            }
        }
    }
    ...
    {
        char name[16] = {};
        uint8_t counts[SAVE_DATA_CARD_COUNT_V11] = {};
        int32_t highest_score = 0;
        uint8_t trinkets[CAMPAIGN_TRINKET_SLOTS] = {
            uint8_t(TrinketType::NONE),
            uint8_t(TrinketType::NONE),
    ...
    struct SaveDataV11
    {
        ...
        int16_t same_number_used_targets[SAME_NUMBER_USED_CAPACITY] = {};
        int32_t number_now_round_best[CAMPAIGN_NUMBER_NOW_ROUNDS] = {};
        uint8_t library_counts[SAVE_DATA_CARD_COUNT_V11] = {};
        uint8_t trinket_owned[SAVE_DATA_TRINKET_COUNT_V10] = {};
        InstancePool instance_pool{};
        SavedDeckV11 decks[MAX_SAVED_DECKS] = {};
    };

    void save_data_migrate_deck_v11_to_v12(SavedDeck& deck, const SavedDeckV11& old_deck)
    {
        for(int index = 0; index < 16; ++index)
        {
            deck.name[index] = old_deck.name[index];
        ...
        save_data_migrate_counts_v11_to_v12(deck.counts, old_deck.counts);
    }

    void save_data_migrate_v11_to_v12(SaveData& data, const SaveDataV11& old_data)
    {
        data = SaveData{};
        data.magic = old_data.magic;
        data.version = SAVE_DATA_VERSION;
        ...
        save_data_migrate_counts_v11_to_v12(data.library_counts, old_data.library_counts);
        ...
        for(int deck_index = 0; deck_index < data.deck_count; ++deck_index)
        {
            save_data_migrate_deck_v11_to_v12(data.decks[deck_index], old_data.decks[deck_index]);
        }
    }

    bool save_data_valid_v11(const SaveDataV11& data)
    {
        if(data.magic != SAVE_DATA_MAGIC || data.version != SAVE_DATA_VERSION_V11 ||
        ...
        for(int deck_index = 0; deck_index < data.deck_count; ++deck_index)
        {
            const SavedDeckV11& deck = data.decks[deck_index];
            int total = 0;

            for(int type_index = 0; type_index < SAVE_DATA_CARD_COUNT_V11; ++type_index)
            {
                total += deck.counts[type_index];
            }
    ...
    {
        char name[16] = {};
        uint8_t counts[SAVE_DATA_CARD_COUNT_V12] = {};
        int32_t highest_score = 0;
        uint8_t trinkets[CAMPAIGN_TRINKET_SLOTS] = {
            uint8_t(TrinketType::NONE),
            uint8_t(TrinketType::NONE),
    ...
    struct SaveDataV12
    {
        ...
        int16_t same_number_used_targets[SAME_NUMBER_USED_CAPACITY] = {};
        int32_t number_now_round_best[CAMPAIGN_NUMBER_NOW_ROUNDS] = {};
        uint8_t library_counts[SAVE_DATA_CARD_COUNT_V12] = {};
        uint8_t trinket_owned[SAVE_DATA_TRINKET_COUNT_V10] = {};
        InstancePool instance_pool{};
        SavedDeckV12 decks[MAX_SAVED_DECKS] = {};
    };

    void save_data_migrate_counts_v12_to_v13(uint8_t* out_counts, const uint8_t* old_counts)
    {
        for(int type_index = 0; type_index < int(CardType::COUNT); ++type_index)
        {
            out_counts[type_index] = type_index < SAVE_DATA_CARD_COUNT_V12 ? old_counts[type_index] : 0;
        }
    }

    void save_data_migrate_deck_v12_to_v13(SavedDeck& deck, const SavedDeckV12& old_deck)
    {
        for(int index = 0; index < 16; ++index)
        {
            deck.name[index] = old_deck.name[index];
        ...
        save_data_migrate_counts_v12_to_v13(deck.counts, old_deck.counts);
    }

    void save_data_migrate_v12_to_v13(SaveData& data, const SaveDataV12& old_data)
    {
        data = SaveData{};
        data.magic = old_data.magic;
        data.version = SAVE_DATA_VERSION;
        ...
        save_data_migrate_counts_v12_to_v13(data.library_counts, old_data.library_counts);
        ...
        for(int deck_index = 0; deck_index < data.deck_count; ++deck_index)
        {
            save_data_migrate_deck_v12_to_v13(data.decks[deck_index], old_data.decks[deck_index]);
        }
    }

    bool save_data_valid_v12(const SaveDataV12& data)
    {
        if(data.magic != SAVE_DATA_MAGIC || data.version != SAVE_DATA_VERSION_V12 ||
           data.deck_count > MAX_SAVED_DECKS)
        {
            return false;
        }
        ...
        for(int deck_index = 0; deck_index < data.deck_count; ++deck_index)
        {
            const SavedDeckV12& deck = data.decks[deck_index];
            int total = 0;

            for(int type_index = 0; type_index < SAVE_DATA_CARD_COUNT_V12; ++type_index)
            {
                total += deck.counts[type_index];
            }
    ...
    {
        char name[16] = {};
        uint8_t counts[SAVE_DATA_CARD_COUNT_V13] = {};
        int32_t highest_score = 0;
        uint8_t trinkets[CAMPAIGN_TRINKET_SLOTS] = {
            uint8_t(TrinketType::NONE),
            uint8_t(TrinketType::NONE),
    ...
    struct SaveDataV13
    {
        ...
        int16_t same_number_used_targets[SAME_NUMBER_USED_CAPACITY] = {};
        int32_t number_now_round_best[CAMPAIGN_NUMBER_NOW_ROUNDS] = {};
        uint8_t library_counts[SAVE_DATA_CARD_COUNT_V13] = {};
        uint8_t trinket_owned[int(TrinketType::COUNT)] = {};
        InstancePool instance_pool{};
        SavedDeckV13 decks[MAX_SAVED_DECKS] = {};
    };

    void save_data_migrate_counts_v13_to_v14(uint8_t* out_counts, const uint8_t* old_counts)
    {
        for(int type_index = 0; type_index < int(CardType::COUNT); ++type_index)
        {
            out_counts[type_index] = type_index < SAVE_DATA_CARD_COUNT_V13 ? old_counts[type_index] : 0;
        }
    }

    void save_data_migrate_deck_v13_to_v14(SavedDeck& deck, const SavedDeckV13& old_deck)
    {
        for(int index = 0; index < 16; ++index)
        {
            deck.name[index] = old_deck.name[index];
        ...
        save_data_migrate_counts_v13_to_v14(deck.counts, old_deck.counts);
    }

    void save_data_migrate_v13_to_v14(SaveData& data, const SaveDataV13& old_data)
    {
        data = SaveData{};
        data.magic = old_data.magic;
        data.version = SAVE_DATA_VERSION;
        ...
        save_data_migrate_counts_v13_to_v14(data.library_counts, old_data.library_counts);
        ...
        for(int deck_index = 0; deck_index < data.deck_count; ++deck_index)
        {
            save_data_migrate_deck_v13_to_v14(data.decks[deck_index], old_data.decks[deck_index]);
        }
    }
    ...
    void save_data_migrate_v16_to_v17(SaveData& data, const SaveDataV16& old_data)
    {
        data = SaveData{};
        data.magic = old_data.magic;
        data.version = SAVE_DATA_VERSION;
        ...
        {
            if(old_data.poker_hand_record[index] > data.poker_hand_record)
            {
                data.poker_hand_record = old_data.poker_hand_record[index];
            }
        }
    }

    bool save_data_valid_v16(const SaveDataV16& data)
    {
        if(data.magic != SAVE_DATA_MAGIC || data.version != SAVE_DATA_VERSION_V16 ||
           data.deck_count > MAX_SAVED_DECKS)
        {
            return false;
        }
    ...
    void save_data_migrate_v17_to_v18(SaveData& data, const SaveDataV17& old_data)
    {
        data = SaveData{};
        data.magic = old_data.magic;
        data.version = SAVE_DATA_VERSION;
    ...
    bool save_data_valid_v17(const SaveDataV17& data)
    {
        if(data.magic != SAVE_DATA_MAGIC || data.version != SAVE_DATA_VERSION_V17 ||
           data.deck_count > MAX_SAVED_DECKS)
        {
            return false;
        }
    ...
    {
        char name[16] = {};
        uint8_t counts[SAVE_DATA_CARD_COUNT_V18] = {};
        int32_t highest_score = 0;
        uint8_t trinkets[CAMPAIGN_TRINKET_SLOTS] = {
            uint8_t(TrinketType::NONE),
            uint8_t(TrinketType::NONE),
    ...
    struct SaveDataV18
    {
        ...
        int16_t same_number_used_targets[SAME_NUMBER_USED_CAPACITY] = {};
        int32_t number_now_round_best[CAMPAIGN_NUMBER_NOW_ROUNDS] = {};
        uint8_t library_counts[SAVE_DATA_CARD_COUNT_V18] = {};
        uint8_t trinket_owned[int(TrinketType::COUNT)] = {};
        InstancePool instance_pool{};
        SavedDeckV18 decks[MAX_SAVED_DECKS] = {};
        uint16_t sticker_paper = 0;
        int32_t aint_got_time_record = 0;
        int32_t sharing_is_caring_record = 0;
        int32_t poker_hand_record = 0;
    ...
    void save_data_migrate_counts_v18_to_v19(uint8_t* out_counts, const uint8_t* old_counts)
    {
        for(int type_index = 0; type_index < int(CardType::COUNT); ++type_index)
        {
            out_counts[type_index] = type_index < SAVE_DATA_CARD_COUNT_V18 ? old_counts[type_index] : 0;
        }
    }

    void save_data_migrate_deck_v18_to_v19(SavedDeck& deck, const SavedDeckV18& old_deck)
    {
        for(int index = 0; index < 16; ++index)
        {
            deck.name[index] = old_deck.name[index];
        ...
        {
            deck.longsleeve_instance_ids[slot] = old_deck.longsleeve_instance_ids[slot];
        }

        save_data_migrate_counts_v18_to_v19(deck.counts, old_deck.counts);
    }
    ...
    void save_data_migrate_v19_to_v20(SaveData& data, const SaveDataV19& old_data)
    {
        data = SaveData{};
        data.magic = old_data.magic;
        data.version = SAVE_DATA_VERSION;
    ...
    void save_data_migrate_v18_to_v19(SaveData& data, const SaveDataV18& old_data)
    {
        ...
        save_data_migrate_counts_v18_to_v19(data.library_counts, old_data.library_counts);
        ...
        for(int deck_index = 0; deck_index < data.deck_count; ++deck_index)
        {
            save_data_migrate_deck_v18_to_v19(data.decks[deck_index], old_data.decks[deck_index]);
        }
    ...
    bool save_data_valid_v18(const SaveDataV18& data)
    {
        if(data.magic != SAVE_DATA_MAGIC || data.version != SAVE_DATA_VERSION_V18 ||
           data.deck_count > MAX_SAVED_DECKS)
        {
            return false;
        }
    ...
    bool save_data_valid_v13(const SaveDataV13& data)
    {
        ...
        for(int deck_index = 0; deck_index < data.deck_count; ++deck_index)
        {
            const SavedDeckV13& deck = data.decks[deck_index];
            int total = 0;

            for(int type_index = 0; type_index < SAVE_DATA_CARD_COUNT_V13; ++type_index)
            {
                total += deck.counts[type_index];
            }
...
void save_data_init()
{
    ...
    else if(!save_data_try_migrate_legacy())
    {
        save_data_reset();
    }
...
void saved_deck_scrub_counts(const SaveData& save, SavedDeck& deck)
{
    for(int type_index = 0; type_index < int(CardType::COUNT); ++type_index)
    {
        ...
        int allowed = max_copies;

        if(!saved_deck_unrestricted_build(deck))
        {
            allowed = save.library_counts[type_index];
        }

        if(deck.counts[type_index] > allowed)
        {
            deck.counts[type_index] = uint8_t(allowed);
        }
    }
}
...
void saved_deck_trim_to_max(SavedDeck& deck, int max_cards)
{
    ...
    {
        for(int type_index = int(CardType::COUNT) - 1; type_index >= 0 && total > max_cards; --type_index)
        {
...
void save_data_validate(SaveData& save)
{
    ...
    {
        campaign_sanitize_npc_collections(save);
    }
    ...
    for(int deck_index = 0; deck_index < save.deck_count; ++deck_index)
    {
        ...
        saved_deck_sanitize_name(deck);
        saved_deck_scrub_counts(save, deck);
        saved_deck_trim_to_max(deck, DECK_MAX_CARDS);
    }

    if(save.campaign_ready != 0)
    {
        bool any_playable_deck = false;

        for(int deck_index = 0; deck_index < save.deck_count; ++deck_index)
        {
            if(saved_deck_total_cards(save.decks[deck_index]) >= DECK_MIN_CARDS)
            {
                any_playable_deck = true;
                break;
            }
        }

        if(!any_playable_deck)
        {
            save.campaign_ready = 0;
        }
    }
