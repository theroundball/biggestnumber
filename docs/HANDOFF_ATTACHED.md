...
**Future goal:** RPG overworld with NPC battles and loot. **That work is blocked** until the card-game refactor exit criteria pass (see §5).
...
**Primary direction (2026-08-12):** Roguelike run mode — overworld scrapped. Use the bootstrap prompt in **`docs/ROGUELIKE_RUN_HANDOFF.md` §1** (copy that block into a new Cursor chat).

The block below is **refactor-only** context (#2d-hand play resolution). Use it only if Garrett asks to continue the battle refactor, not for roguelike run work.

---

### Refactor bootstrap (secondary — #2d-hand only)

Paste this into a new Cursor chat on another machine after syncing the repo:

You are continuing **Biggest Number** (Butano GBA card game). Read `docs/HANDOFF.md` first, then `docs/play_resolution_matrix.md` and `include/play_resolution.h`.

**Current state:** Refactor steps #4, #5, #2a, #2b, #2c are done. `route_played_card()` is implemented but **not wired to call sites**. Duplicate play paths still exist in `game_input.cpp` and `game_helpers.cpp`.

**Your task:** Continue with **#2d-hand** — migrate the hand play path in `game_input.cpp` to use `route_played_card()`. Run manual test matrix rows 1–5, 9, 11 before considering done.

**Do not:** start overworld work, commit without being asked, or merge hand + scry + echo in one PR.

**Key design rules:** Unified Echo (full second play after idle, no Swivel/Hacker special cases); Burger = early GY cost; Clover = held cost; Bones counts self in GY size; both Echo instances increment `cards_played_this_round`.

Ask before large architectural changes outside the refactor plan.
